...
export const CareerSection: FC = () => {
  return (
    <section
      ...
      className="scroll-mt-24 border-t border-border/70 bg-[#f7f8f8] py-section text-foreground"
    >
              ...
              <dl className="mt-4 space-y-3">
                {CAREER_QUALIFICATIONS.map((q) => (
                  <div key={q.text} className="grid grid-cols-[5rem_1fr] gap-4">
                    <dt className="text-body-1 tabular-nums">
                      {q.year}
                    </dt>
                    <dd className="break-keep text-body-1 text-muted-foreground">{q.text}</dd>
                  </div>
                ))}
              </dl>
            </div>
