# Payload Website Template — 원본 README 보관본

> 본 파일은 `lawyer-ryu` 프로젝트의 베이스가 된 **Payload Website Template** 의 원본 `README.md` 를 그대로 보관한 사본이다.
> 류남경 변호사 홈페이지로 전환되는 과정에서 루트 `README.md` 가 한국어 프로젝트용으로 교체되었으며, 템플릿이 제공하던 기술 가이드(인증·드래프트·라이브 프리뷰·SEO·검색·리다이렉트·잡 큐·Postgres·Vercel 배포 등)의 원문을 잃지 않도록 복사해 둔다.
> 보관 시점: 2026-05-05.
